...
// ----------------- Public view -----------------------------------------------
    ...
    public function delete_program_class($program_class_id){
		...
		$room_availability = $this->room_availability_model->get_room_availability_related_with_program($program_class_id)->result();
			...
			$this->session->set_userdata('error_messege','Program '.$value[0]->NAMA_PROGRAM.' masih digunakan pada data lain');
			}
			else{
			 ...
			 $this->session->set_userdata('notification_messege','Program '.$value[0]->NAMA_PROGRAM.' berhasil dihapus');
